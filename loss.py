...
class MinimumPixelLoss():
    ...
    def forward(self, batch, G_pred1, G_pred2):
        ...
        morph_1= np.array((batch['input']).cpu().detach())
        morph_1 = morph_1[0,:].transpose([1,2,0])
        
        t_1 = np.array((batch['gt1']).cpu().detach())
        t_1 = t_1[0, :].transpose([1, 2, 0])
        t_2 = np.array((batch['gt2']).cpu().detach())
        t_2 = t_2[0, :].transpose([1, 2, 0])
...
class BiometricLoss(nn.Module):
    
    def __init__(self, th = 0.68):
        self.th = th
    ...
    def forward(self, batch, G_pred1, G_pred2):
        ...
        tar1 = np.array((batch['gt1']).cpu().detach())
        tar1 = tar1[0, :].transpose([1, 2, 0])
        tar2 = np.array((batch['gt2']).cpu().detach())
        tar2 = tar2[0, :].transpose([1, 2, 0])
        
        morph= np.array((batch['input']).cpu().detach())
        morph = morph[0,:].transpose([1,2,0])
       
        loss_11 = DeepFace.verify(img1_path = G_pred1, img2_path = tar1,model_name = "ArcFace",enforce_detection=False) 
        loss_22 = DeepFace.verify(img1_path = G_pred2, img2_path = tar2,model_name = "ArcFace",enforce_detection=False)
        loss_12 = DeepFace.verify(img1_path = G_pred1, img2_path = tar2,model_name = "ArcFace",enforce_detection=False)                   
        loss_21 = DeepFace.verify(img1_path = G_pred2, img2_path = tar1,model_name = "ArcFace",enforce_detection=False)
        
        loss_ = min(loss_11['distance'] + loss_22['distance'], loss_12['distance'] + loss_21['distance'])         
