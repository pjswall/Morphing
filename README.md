...
Collect the morphed dataset from the original authors and then split into train and test sets using. You can split the train and test code as per labelling of data for example if label is like 01_02_45N it can be interpreted as 1st set of images morphed with 2nd set and the serial no. used is 45. In similar order you can split your train and test data. Please refer to the paper for training and test distributions. Moreover we have also used MTCNN face cropper for properly segmenting the face of subjects so that we will get better results. In this repository, we have provided supporting code for the AMSL dataset, you can follow the same process for other datasets. We followed the filenaming convention provided in [github repo](https://github.com/jiupinjia/Deep-adversarial-decomposition). Ensure that when image pairs are provided for training, both morphed and ground-truth images used in creating the morph have the same filename.  
...
If you find any bugs in this code or experience issues please report it to Issues page on the Github repo or email prateek.jaiswal@research.iiit.ac.in
