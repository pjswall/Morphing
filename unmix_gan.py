...
https://github.com/jiupinjia/Deep-adversarial-decomposition/blob/master/eval_unmix.py
"""
...
class UnmixGAN():
    ...
    def __init__(self, args, dataloaders):
        ...
        self.lambda_bio = args.lambda_bio
        #------We will need 1 parameter for biometric loss----#
        ...
        # define the loss functions  -------Load library(Deep face/model(ArcFace))--------
        ...
        self._biometric_loss = loss.BiometricLoss() #Added biometric loss 
        # enable some losses?
        ...
        self.with_biometric_loss =args.enable_biometric_loss #Enabled biometric loss
        ...
        if os.path.exists(self.checkpoint_dir) is False:
            os.mkdir(self.checkpoint_dir)                             #Making Checkpoint Dir
    ...
    def _backward_G(self):  
        ...
        biometric_loss = torch.tensor(0.0, requires_grad=True).to(device)
        #biometric_loss = library.module(**kwargs).to(device)
        
        if self.with_biometric_loss:
            biometric_loss = self._biometric_loss.forward(
                batch=self.batch, G_pred1=self.G_pred1, G_pred2=self.G_pred2)
        ...
        # print("BIO",self.lambda_bio*biometric_loss)
        # print("Pixel",self.lambda_L1*pixel_loss)
        # print("ADV",self.lambda_adv*G_adv_loss)
        # print("EXCLU",2*exclusion_loss)5
        # print(kurtosis_loss)
        # print(biometric_loss)
        
        self.G_loss = self.lambda_L1*pixel_loss+self.lambda_adv*G_adv_loss + 2*exclusion_loss + kurtosis_loss + self.lambda_bio*biometric_loss
        
        # print(self.G_loss)
        # print(self.G_loss.sum())
        
        G = np.array(self.G_loss.cpu().detach())
        plt.plot(np.array(G),'r')
        self.G_loss.sum().backward()
